...
int connectClient(lua_State* L) {
  ...
  int send_timeout_ms = (nargs > 2 ? luaL_checkint(L, 4) : -1);
  int receive_timeout_ms = (nargs > 3 ? luaL_checkint(L, 5) : -1);
  if (!cl->connect(hostname, port, send_timeout_ms, receive_timeout_ms)) {
